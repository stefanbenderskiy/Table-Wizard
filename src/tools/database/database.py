...
class DataBase:
    ...
    def update(self, table, value, column, item = None, indificator = "id"):
        if item:
            return self.get_request("""UPDATE ? SET ? = ?""", (table, column, value))
        else:
            return self.get_request("""UPDATE ? SET ? = ? WHERE ? = ?""", (table, column,value, indificator, item))
